...
Use this skill to review normalized VoiceNotes notes and recommend the next step. This skill is analysis-only by default. It must not send, move, or dispatch a note to any downstream project without explicit user confirmation in the current conversation.
...
4. Before any dispatch proposal, make sure there is a compiled package with a concise summary, extracted tasks, decisions, open questions, and evidence spans:
   - `python3 scripts/voice_notes.py compile --note-id <source_note_id>`
5. If the outcome is `propose_dispatch`, stop and ask for confirmation. Do not write the note yet.
6. If multiple pending notes appear to describe the same theme, run `python3 scripts/voice_notes.py discover` and use the suggested buckets and relationships before proposing a new project or rule.
