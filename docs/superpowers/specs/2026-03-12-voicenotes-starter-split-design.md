...
| Client canonical path | `src/voice_notes/sync_client.py` | Neutral, alongside existing CLI module |
...
### VoiceNotes-template (private GitHub template repo)
...
### VoiceNotes-private (private, created from current state)
...
**Files to modify:** `src/voice_notes/cli.py` (dispatch validation logic, dispatch summary output)

### 2. Client Migration

Move canonical VoiceNotes API client from `.codex/skills/voicenotes-direct-sync/scripts/voicenotes_client.py` to `src/voice_notes/sync_client.py`.

- `scripts/voicenotes_client.py` becomes thin wrapper importing from `src.voice_notes.sync_client`
- `.codex/skills/.../scripts/voicenotes_client.py` is removed
- Both skill surfaces reference `python3 scripts/voicenotes_client.py`
- Tests update `importlib.util` path to new location

**Path calculation change:** The current client calculates `REPO_ROOT = Path(__file__).resolve().parents[4]` (because it lives 4 levels deep under `.codex/skills/.../scripts/`). After moving to `src/voice_notes/sync_client.py`, this must become `parents[2]`.

**Test helper change:** `load_sync_module()` in `tests/test_voice_notes.py` currently hardcodes the `.codex/skills/.../scripts/voicenotes_client.py` path via `importlib.util`. Must update to load from `src/voice_notes/sync_client.py`.

**Wrapper implementation:** `scripts/voicenotes_client.py` (~10 lines) adds repo root to `sys.path` and delegates to `src.voice_notes.sync_client.main()`. Same pattern as `scripts/voice_notes.py`.

**Files to create:** `src/voice_notes/sync_client.py`
**Files to modify:** `scripts/voicenotes_client.py`, `.codex/skills/voicenotes-direct-sync/SKILL.md`, `.claude/skills/voicenotes-direct-sync/SKILL.md`, `tests/test_voice_notes.py` (wrapper + `load_sync_module()` path)
**Files to remove:** `.codex/skills/voicenotes-direct-sync/scripts/voicenotes_client.py`
...
- Verify command references in both surfaces use `scripts/voicenotes_client.py` (not `.codex/...` paths)
...
- Neutral entrypoint `scripts/voicenotes_client.py` used (no `.codex/` or `.claude/` internal paths)
...
```json
{
  "template_name": "voicenotes-triage-hub",
  "version": "0.1.0",
  "min_python": "3.10"
}
```
...
- Session defaults reference `scripts/voicenotes_client.py`
...
2. Create `VoiceNotes-template-work` — copy of current state for neutralization
3. Create `VoiceNotes-private-work` — copy of current state preserving everything real

### Phase 2: Close the Private Repo First

In `VoiceNotes-private-work`:
...
In `VoiceNotes-template-work`:
1. Replace branded project keys with neutral examples in `registry.shared.json`
2. Move client implementation to `src/voice_notes/sync_client.py`
3. Update wrapper in `scripts/voicenotes_client.py`
...
1. Push `VoiceNotes-template` to GitHub
2. Enable: Template Repository, release automation, parity checks
3. Push `VoiceNotes-private` to GitHub (private)
...
### Neutral Client
- `python3 scripts/voicenotes_client.py --help` works without `.codex/` dependency
- `python3 scripts/voicenotes_client.py sync --output-dir ./data/raw` works from `src/` module
