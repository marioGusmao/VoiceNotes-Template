...
Use this skill to interact with Voicenotes directly from Codex via the OpenClaw-compatible integration token. Prefer the bundled script instead of ad-hoc `curl` so requests, canonical raw JSON export, Markdown rendering, and auth handling stay consistent.
...
2. Use the repository entrypoint `scripts/voicenotes_client.py` for all Voicenotes operations.
...
```bash
python3 scripts/voicenotes_client.py list --from 2026-03-01T00:00:00Z
python3 scripts/voicenotes_client.py search --query "home renovation idea"
python3 scripts/voicenotes_client.py get --id NTHiJljf --format markdown
python3 scripts/voicenotes_client.py sync --output-dir ./data/raw
python3 scripts/voicenotes_client.py create --text "Remember to review the warranty"
```
